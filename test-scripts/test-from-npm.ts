...
/**
 * Test Script: Import from npm (RC Version)
 *
 * Tests the library imported directly from npm to validate the published package.
 * This test confirms that the npm package works as expected.
 */
...
import { ConsoleStructuredLogger, LogLevel } from '@luoarch/baileys-store-core';
import { withContext } from '@luoarch/baileys-store-core';
import qrcode from 'qrcode-terminal';
import { Boom } from '@hapi/boom';
import { MongoClient } from 'mongodb';
import Redis from 'ioredis';

const SESSION_ID = 'test-npm-import-session';
...
async function autoCleanup(): Promise<void> {
  logger.info('Iniciando auto-cleanup', { adapter: 'hybrid-npm', sessionId: SESSION_ID });
  ...
  try {
    ...
    logger.info('Auto-cleanup concluído', { adapter: 'hybrid-npm', sessionId: SESSION_ID });
  } catch (error) {
...
/**
 * Connect with Hybrid auth state imported from npm
 */
async function connectWithAuth(): Promise<void> {
  logger.info('Iniciando conexão com biblioteca do npm', { sessionId: SESSION_ID });

  try {
    // Initialize auth state only once
    if (!globalAuthState) {
      logger.info('Inicializando auth state do npm', {
        adapter: 'hybrid-npm',
        sessionId: SESSION_ID,
      });
      ...
      logger.info('Auth state inicializado do npm', {
        adapter: 'hybrid-npm',
        sessionId: SESSION_ID,
        encryption: true,
        compression: true,
    ...
    sock.ev.on('connection.update', (update) => {
      ...
      // Handle successful connection
        ...
        console.log('\n' + '━'.repeat(80));
        console.log('✨ SUCESSO: Biblioteca do npm está funcionando corretamente!');
        console.log('━'.repeat(80) + '\n');
        ...
        console.log(`   - Source: npm @rc package 📦\n`);
        console.log('🎯 Validação:');
        console.log('   ✅ Biblioteca importada do npm (@luoarch/baileys-store-core)');
        console.log('   ✅ Versão: 1.0.0-rc.1');
        console.log('   ✅ Funcionalidade híbrida operacional');
        ...
        console.log('   3. Confirme que a biblioteca npm está funcionando');
    ...
    console.error('\n💡 Troubleshooting:');
    console.error('   1. Verifique se instalou: npm install @luoarch/baileys-store-core@rc');
    console.error('   2. Verifique Redis: redis-cli ping');
    console.error('   3. Verifique MongoDB: mongosh --eval "db.runCommand({ping: 1})"');
    console.error('   4. Verifique imports do npm estão corretos');
    console.error('   5. Verifique logs acima para detalhes\n');
    process.exit(1);
  }
}
...
async function main(): Promise<void> {
  console.log('\n' + '='.repeat(80));
  console.log('🧪 TEST: Import from npm (@luoarch/baileys-store-core@rc)');
  console.log('='.repeat(80) + '\n');
