/**
 * @baileys-store/core - Hybrid + Kafka Example
 *
 * Example using Hybrid storage with Kafka for write-behind
 * - Redis as hot cache for low latency
 * - MongoDB as cold storage for durability
 * - Kafka for async write-behind processing
 * - QueueAdapter implementation for Kafka
 */
...
async function main() {
  console.log('🚀 Starting WhatsApp with Hybrid auth state + Kafka write-behind...');

  // Create Kafka client
  const kafka = new Kafka({
    clientId: 'baileys-hybrid-auth',
  ...
  const kafkaAdapter = new KafkaAdapter(kafka);
  ...
  // Create hybrid store with Kafka queue
  ...
  // Create Kafka consumer for write-behind processing
  const consumer = kafka.consumer({ groupId: 'baileys-auth-persist-group' });
  await consumer.connect();
  await consumer.subscribe({ topic: 'baileys-auth-persist', fromBeginning: false });

  await consumer.run({
    eachMessage: async ({ topic, partition, message }) => {
      try {
        const payload = JSON.parse(message.value?.toString() || '{}');
        const { sessionId, patch, version, fencingToken } = payload.data;

        console.log('📝 Processing Kafka write-behind message:', {
          sessionId,
          version,
          partition,
        ...
        console.log('✅ Kafka write-behind completed:', message.offset);
      } catch (error) {
        console.error('❌ Kafka write-behind failed:', error);
  ...
  const authState = {
    ...
    keys: {
      get: async (type: string, ids: string[]) => {
        const snapshot = await hybridStore.get('my-hybrid-kafka-session');
        if (!snapshot) return {};
      ...
      set: async (data: Record<string, Record<string, unknown>>) => {
        ...
        await hybridStore.set('my-hybrid-kafka-session', { keys: mergedKeys }, currentVersion);
      },
    },
  };
  ...
  socket.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log('✅ WhatsApp connected successfully with Hybrid + Kafka!');
      console.log('📊 Kafka write-behind processing is active');
    }
  });
  ...
  socket.ev.on('creds.update', async () => {
    const snapshot = await hybridStore.get('my-hybrid-kafka-session');
    const currentVersion = snapshot?.version || 0;
    await hybridStore.set('my-hybrid-kafka-session', { creds: authState.creds }, currentVersion);
  });
